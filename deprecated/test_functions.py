...
def _check_modal_tokens() -> bool:
    ...
    if not bool(os.getenv("MODAL_TOKEN_ID") and os.getenv("MODAL_TOKEN_SECRET")):
        raise ValueError(
            "Modal tokens not configured; skipping Modal integration tests"
        )
    return True
...
def test_dataset_functions_flow_write_list_then_optional_export():
    ...
    res = write_dataset_bytes.remote(
        dataset_id="unit", data_b64=b64, session_id=session_id, ext="csv"
    )
    ...
    print("Wrote dataset to sandbox successfully: ", res)
    ...
    found = False
    names = set()
    for _ in range(20):  # up to ~10s to allow Volume sync
        files = list_loaded_datasets.remote(session_id=session_id)
    ...
    print("unit.csv found in listed datasets: ", names)
    ...
    if have_s3:
        uploaded = export_dataset.remote(
            "datasets/unit.csv", bucket=os.environ["S3_BUCKET"], session_id=session_id
        )
        assert uploaded.get("s3_key") and uploaded.get("s3_url")
        print("Uploaded to S3 Successfully! URL: ", uploaded.get("s3_url"))
