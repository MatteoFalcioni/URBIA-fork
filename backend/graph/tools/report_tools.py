...
@tool(
    ...
    description="Use this to assign the task to the report writer."
)
def assign_to_report_writer_tool(
    reason: Annotated[str, "Brief reason why the report writer should be assigned"],
    runtime: ToolRuntime
) -> Command:
    """
    Assigns the task to the report writer. 
    ...
    return Command(
        update={
            "messages": [ToolMessage(
                content=f"Assigning to report writer because of the following reason: {reason}", 
